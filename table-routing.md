...
| POST | `/events` | **Required** (Organizer) | Create new event | - | `{ title, description, location, startDate, endDate, price, seatTotal, category, imageUrl?, isPromoted? }` |
...
| POST | `/events/:eventId/vouchers` | **Required** (Organizer) | Create voucher for event | - | `{ code, discount, quantity, expiresAt }` |
