...
export const createVoucherSchema = yup.object({
  code: yup.string().required("Code is required").min(3, "Code must be at least 3 characters").uppercase(),
  discountAmount: yup.number().nullable().optional().min(0, "Discount amount must be positive"),
  discountPercent: yup.number().nullable().optional().min(1, "Discount percent must be at least 1%").max(100, "Discount percent cannot exceed 100%"),
  maxUsage: yup.number().default(100).min(1, "Max usage must be at least 1"),
  startDate: yup.date().required("Start date is required"),
  endDate: yup.date().required("End date is required").min(yup.ref('startDate'), "End date must be after start date"),
}).test('at-least-one-discount', 'Either discountAmount or discountPercent is required', function(value) {
